    ...
    def update(self, observations):
        ...
        self.filtered_state_means, self.filtered_state_covariances = (
            self.kf.filter_update(
                self.filtered_state_means, self.filtered_state_covariances, observations
            )
        )
        ...
        delta_dist_travelled = (
            ...
            + (self.pos_y - self.filtered_state_means[1]) ** 2
        ) ** (1.0 / 2.0)
...
class KalmanMultiTrackerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("publish_occluded", True)
        self.declare_parameter(
            "publish_people_frame", "laser"
        )  # Using default from fixed_frame
        self.declare_parameter("use_scan_header_stamp_for_tfs", False)
        self.declare_parameter("publish_detected_people", False)
        ...
        self.max_leg_pairing_dist = (
            self.get_parameter("max_leg_pairing_dist")
            .get_parameter_value()
            .double_value
        )
        self.confidence_threshold_to_maintain_track = (
            self.get_parameter("confidence_threshold_to_maintain_track")
            .get_parameter_value()
            .double_value
        )
        self.publish_occluded = (
            self.get_parameter("publish_occluded").get_parameter_value().bool_value
        )
        self.publish_people_frame = (
            self.get_parameter("publish_people_frame")
            .get_parameter_value()
            .string_value
        )
        self.use_scan_header_stamp_for_tfs = (
            self.get_parameter("use_scan_header_stamp_for_tfs")
            .get_parameter_value()
            .bool_value
        )
        self.publish_detected_people = (
            self.get_parameter("publish_detected_people")
            .get_parameter_value()
            .bool_value
        )
        self.dist_travelled_together_to_initiate_leg_pair = (
            self.get_parameter("dist_travelled_together_to_initiate_leg_pair")
            .get_parameter_value()
            .double_value
        )
        scan_topic = self.get_parameter("scan_topic").get_parameter_value().string_value
        self.scan_frequency = (
            self.get_parameter("scan_frequency").get_parameter_value().double_value
        )
        self.in_free_space_threshold = (
            self.get_parameter("in_free_space_threshold")
            .get_parameter_value()
            .double_value
        )
        self.confidence_percentile = (
            self.get_parameter("confidence_percentile")
            .get_parameter_value()
            .double_value
        )
        self.max_std = self.get_parameter("max_std").get_parameter_value().double_value

        self.mahalanobis_dist_gate = scipy.stats.norm.ppf(
            1.0 - (1.0 - self.confidence_percentile) / 2.0, 0, 1.0
        )
        self.max_cov = self.max_std**2
        self.latest_scan_header_stamp_with_tf_available = self.get_clock().now()

        # ROS publishers
        self.people_tracked_pub = self.create_publisher(
            PersonArray, "people_tracked", 300
        )
        self.people_detected_pub = self.create_publisher(
            PersonArray, "people_detected", 300
        )
        self.marker_pub = self.create_publisher(Marker, "visualization_marker", 300)
        self.non_leg_clusters_pub = self.create_publisher(
            LegArray, "non_leg_clusters", 300
        )

        # ROS subscribers
        self.detected_clusters_sub = self.create_subscription(
            LegArray, "detected_leg_clusters", self.detected_clusters_callback, 300
        )
        self.local_map_sub = self.create_subscription(
            OccupancyGrid, "local_map", self.local_map_callback, 300
        )
    ...
    def how_much_in_free_space(self, x, y):
        ...
        map_x = int(
            round(
                (x - self.local_map.info.origin.position.x)
                / self.local_map.info.resolution
            )
        )
        map_y = int(
            round(
                (y - self.local_map.info.origin.position.y)
                / self.local_map.info.resolution
            )
        )
        ...
        for i in range(map_x - kernel_size, map_x + kernel_size):
            for j in range(map_y - kernel_size, map_y + kernel_size):
                if i + j * self.local_map.info.height < len(self.local_map.data):
                    ...
                    return self.in_free_space_threshold * 2
        percent = sum / (((2.0 * kernel_size + 1) ** 2.0) * 100.0)
        ...
        for detect in objects_detected:
            ...
            for track in objects_tracked:
                ...
                if track.is_person and not detect.in_free_space_bool:
                    ...
                    cov = (
                        track.filtered_state_covariances[0][0] + track.var_obs
                    )  # cov_xx == cov_yy == cov
                    mahalanobis_dist = math.sqrt(
                        (
                            (detect.pos_x - track.pos_x) ** 2
                            + (detect.pos_y - track.pos_y) ** 2
                        )
                        / cov
                    )  # = scipy.spatial.distance.mahalanobis(u,v,inv_cov)**2
    ...
    def detected_clusters_callback(self, detected_clusters_msg):
        ...
        if (
            self.use_scan_header_stamp_for_tfs
        ):  # Assume <self.use_scan_header_stamp_for_tfs> means we're running the timing benchmark
            wait_iters = 0
        ...
        for cluster in detected_clusters_msg.legs:
            new_detected_cluster = DetectedCluster(
                ...
                cluster.position.y,
                cluster.confidence,
                in_free_space=self.how_much_in_free_space(
        ...
        matched_tracks = self.match_detections_to_tracks_GNN(
            propogated, detected_clusters
        )
        ...
        for idx, track in enumerate(self.objects_tracked):
            ...
            if propogated_track.is_person:
                if (
                    propogated_track in matched_tracks
                    and duplicates[propogated_track] in matched_tracks
                ):
                    ...
                    matched_detection = DetectedCluster(
                        (md_1.pos_x + md_2.pos_x) / 2.0,
                        (md_1.pos_y + md_2.pos_y) / 2.0,
                        (md_1.confidence + md_2.confidence) / 2.0,
                        (md_1.in_free_space + md_2.in_free_space) / 2.0,
                    )
            ...
            if matched_detection:
                observations = np.array(
                    [matched_detection.pos_x, matched_detection.pos_y]
                )
                track.in_free_space = (
                    0.8 * track.in_free_space + 0.2 * matched_detection.in_free_space
                )
                track.confidence = (
                    0.95 * track.confidence + 0.05 * matched_detection.confidence
                )
                track.times_seen += 1
                track.last_seen = now
            ...
            if (
                track.is_person
                and track.confidence < self.confidence_threshold_to_maintain_track
            ):
        ...
        for track_1 in self.objects_tracked:
            for track_2 in self.objects_tracked:
                if (
                    ...
                    self.potential_leg_pair_initial_dist_travelled[
                        (track_1, track_2)
                    ] = (track_1.dist_travelled, track_2.dist_travelled)
        ...
        for track_1, track_2 in potential_leg_pairs_list:
            ...
            dist = (
                ...
                + (track_1.pos_y - track_2.pos_y) ** 2
            ) ** (1.0 / 2.0)
            if (
                dist > self.max_leg_pairing_dist
                or track_1.deleted
                or track_2.deleted
            ...
            if track_1.seen_in_current_scan and track_2.seen_in_current_scan:
                track_1_initial_dist, track_2_initial_dist = (
                    self.potential_leg_pair_initial_dist_travelled[(track_1, track_2)]
                )
                dist_travelled = min(
                    track_1.dist_travelled - track_1_initial_dist,
                    track_2.dist_travelled - track_2_initial_dist,
                )
                if (
                    dist_travelled > self.dist_travelled_together_to_initiate_leg_pair
                    and (
                        track_1.in_free_space < self.in_free_space_threshold
                        or track_2.in_free_space < self.in_free_space_threshold
                    )
                ):
                    if not track_1.is_person and not track_2.is_person:
                        ...
                        self.objects_tracked.append(
                            ObjectTracked(
                                (track_1.pos_x + track_2.pos_x) / 2.0,
                                (track_1.pos_y + track_2.pos_y) / 2.0,
                                now,
    ...
    def publish_tracked_objects(self, now):
        ...
        if self.use_scan_header_stamp_for_tfs:
            tf_time = now
            try:
                transform1 = self.buffer.lookup_transform(
                    self.publish_people_frame,
                    self.fixed_frame,
                    tf_time,
                    rclpy.duration(1.0),
            ...
            transform_available = self.buffer.can_transform(
        ...
        if not transform_available:
            ...
            for track in self.objects_tracked:
                ...
                if (
                    self.publish_occluded or track.seen_in_current_scan
    ...
    def publish_tracked_people(self, now):
        ...
        if self.use_scan_header_stamp_for_tfs:
            ...
            transform_available = self.buffer.can_transform(
                self.fixed_frame, self.publish_people_frame, tf_time
            )
        ...
        if not transform_available:
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    if (
                        self.publish_occluded or person.seen_in_current_scan
                    ):  # Only publish people who have been seen in current scan, unless we want to publish occluded people
                        ...
                        cov = (
                            person.filtered_state_covariances[0][0] + person.var_obs
                        )  # cov_xx == cov_yy == cov
                        std = cov ** (1.0 / 2.0)
                        gate_dist_euclid = scipy.stats.norm.ppf(
                            1.0 - (1.0 - self.confidence_percentile) / 2.0, 0, std
                        )
                        marker.pose.position.x = ps.point.x
                        marker.pose.position.y = ps.point.y
                        marker.type = Marker.SPHERE
