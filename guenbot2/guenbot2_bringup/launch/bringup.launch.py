...
def generate_launch_description():
    ...
    joy_launch_path = PathJoinSubstitution(
        [FindPackageShare('guenbot2_bringup'), 'launch', 'joy_teleop.launch.py']
    )
    ...
    ekf_config_path = PathJoinSubstitution(
        [FindPackageShare("guenbot2_base"), "config", "ekf.yaml"]
    )

    default_robot_launch_path = PathJoinSubstitution(
        [FindPackageShare('guenbot2_bringup'), 'launch', 'default_robot.launch.py']
    )

    custom_robot_launch_path = PathJoinSubstitution(
        [FindPackageShare('guenbot2_bringup'), 'launch', 'custom_robot.launch.py']
    )

    extra_launch_path = PathJoinSubstitution(
        [FindPackageShare('guenbot2_bringup'), 'launch', 'extra.launch.py']
    )
